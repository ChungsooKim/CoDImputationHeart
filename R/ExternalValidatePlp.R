...
externalValidatePlp <- function(plpResult,
  ...
  for(i in 1:length(validationSchemaCdm)){
    ...
    newData <- CoDImputationHeart::similarPlpData(plpModel= plpResult$model, createCohorts = F, 
    ...
    if(sum(newData$population$outcomeCount>0)==0){
      warning('Outcome count is 0... Only imputation will be performed')
    }
      if(sum(newData$population$outcomeCount>0) > 0){
      results[[i]] <- CoDImputationHeart::applyModel(population=newData$population, plpData = newData$plpData, 
      ...
      demographics <- as.data.frame(newData$plpData$covariateData$covariates) %>%
        filter(covariateId %in% c(8532001, 1002, seq(3, 19003, by = 1000)))
      demographicsRef <- as.data.frame(newData$plpData$covariateData$covariateRef) %>% 
