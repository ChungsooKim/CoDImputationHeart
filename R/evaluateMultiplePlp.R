...
evaluateMultiplePlp <- function(analysesLocation,
  ...
  for(i in 1:length(modelSettings)){
    ...
    if(dir.exists(file.path(modelSettings[i],'plpResult'))){
      ...
      plpResult <- CoDImputationHeart::loadPlpResult(file.path(modelSettings[i],'plpResult'))
      
      validations <-   tryCatch(CoDImputationHeart::externalValidatePlp(plpResult = plpResult,
